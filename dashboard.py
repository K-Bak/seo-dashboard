...
SHEET_ID = "1sQuYdHhrA23zMO7tqyOFQ_m6uHYsfAr4vg3muXl6K_w"          # SEO-arket
...
# ---------- Konstanter (justér frit) ------------------------------------------
YEAR_GOAL = 350_000      # års-mål (2025)  <<––– JUSTÉR hvis andet tal!
Q3_GOAL   = 120_000      # uge 27-40       <<––– JUSTÉR hvis andet tal!
YEAR      = 2025
START_UGE = 27
SLUT_UGE  = 40
...
mask_q3   = df["Uge"].between(START_UGE, SLUT_UGE) & mask_year
...
# ---------- Layout ------------------------------------------------------------
st.markdown("<h1 style='text-align:center;margin-top:-50px;margin-bottom:-80px'>"
            "SEO – Q3</h1>", unsafe_allow_html=True)
...
with col1:
    ...
    with st.columns([0.05, 0.9, 0.05])[1]:
        ...
        ax.plot([f"Uge {u}" for u in tilbud_ugevis.index],
                tilbud_ugevis.values, ls="--", color="gray", alpha=.5,
                label="Tilbud sendt")
        ax.plot([f"Uge {u}" for u in afslag_ugevis.index],
...
produktliste = ["SEO Ai Boost", "Linkbuilding", "Content pakker",
                "GBP", "Bing Business", "Maps Optimering"]
